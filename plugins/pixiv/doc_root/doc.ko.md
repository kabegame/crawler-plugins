# Pixiv 크롤러 - 플러그인 안내

이 플러그인은 Pixiv에서 일러스트를 가져옵니다. 랭킹, 북마크, 작가 작품, 키워드 검색 네 가지 모드를 지원합니다.

## HTTP 헤더 설정 (필수)

Pixiv API는 인증과 Referer가 필요합니다. **Cookie는 「고급 설정 → HTTP 헤더」에서 설정하세요.** 플러그인 변수로 주입되지 않습니다.

### Cookie 얻는 방법

#### 방법 1: 브라우저에서 복사

1. 브라우저에서 [pixiv.net](https://www.pixiv.net) **로그인** (미가입 시 가입)

![home](./images/home.png)

2. 개발자 도구(F12) 열기 → 네트워크 탭으로 이동 (이미지의 빨간 상자)

![console](./images/console.png)

3. 이미지 순서대로 cookie 복사

![cookie](./images/cookie.png)

4. Kabegame 실행
5. cookie가 필요한 설정에서 HTTP 헤더 추가

![header](./images/header-config.png)

#### 방법 2: Kabegame 서프 페이지에서 복사 (더 쉬움, PC만)

1. Kabegame 실행 후 서프 탭으로 이동

![kabegame-surf](./images/kabegame-surf.png)

2. 플러그인 빠른 입장 → Pixiv 선택 → 「서프 시작」 클릭. Pixiv 창이 열리면 로그인

![surf-pixiv](./images/surf-pixiv.png)

3. 로그인 후 「사이트 cookie 보기」(창 닫지 말 것) 클릭 후 표시된 cookie 복사

![cookie-dialog](./images/cookie-dialog.png)

4. 준비 완료.

### 사용자 ID 얻기 (본인 또는 작가)

Pixiv에서 해당 사용자 프로필을 열면, URL 중간 또는 끝의 숫자가 사용자 ID입니다.

![user](./images/user.png)

### Cookie가 필요한 경우

| 모드 | Cookie |
|------|--------|
| 랭킹(비 R18) | 선택 |
| 랭킹(R18) | 필수 |
| 북마크 | 필수 |
| 작가 작품(비 R18) | 선택 |
| 작가 작품(R18) | 필수 |
| 키워드 검색(비 R18) | 선택 |
| 키워드 검색(R18) | 필수 |

## 크롤 타입

- **랭킹**: 일/주/월 랭킹으로 지정 날짜 작품 다운로드
- **북마크**: 공개 북마크 다운로드
- **작가 작품**: 지정 작가의 공개 작품 다운로드
- **키워드 검색**: 키워드로 검색 후 다운로드

## 설정 항목

선택한 모드에 따라 항목이 바뀝니다.

- **랭킹**: 랭킹 유형, 콘텐츠 유형, 시작일, 종료일(양끝 포함. 달력 선택, 작업 변수는 `YYYYMMDD`)
- **북마크**: 사용자 UID
- **작가**: 사용자 UID, 작가 UID
- **키워드**: 검색 키워드, 검색 모드(세이프/R18/전체), 정렬(날짜/인기). **잘 쓰면 원하는 것만 정확히 받을 수 있습니다**

## 최대 작품 수 (`num_artworks`)

**1～1000** 정수, **네 모드 모두** 사용. **작품(항목) 개수** 기준이며, **다페이지 작품**이면 저장 파일 수는 더 많을 수 있습니다.

스크립트는 **스트리밍**입니다. 목록 API를 **페이지마다** 요청하고, `illust_id`마다 곧바로 `ajax/illust/{id}/pages` 후 원본 다운로드. `num_artworks`에 도달하면 **중단**하며, 상한으로 페이지 수를 미리 계산해 목록만 먼저 긁지 않습니다(빈 페이지 **404** 방지).

### 동작 요약

1. **랭킹** 날짜순, `p=1`부터 JSON. `contents`의 각 ID는 **다운로드한 뒤** 카운트. 실패(**404** 등)면 그날 페이지 종료 후 다음 날. `contents`가 50 미만이면 그날 마지막 페이지.
2. **북마크** `limit=48`로 한 페이지 받고 그 페이지 작품을 **즉시** 다운로드. 상한 또는 끝까지.
3. **작가** `profile/all` **한 번**(목록 페이징 없음). 키 순서로 **받으면서** `num_artworks`에서 멈춤.
4. **키워드** 검색 `p=1,2,…`로 랭킹과 같이 스트리밍.

## 설정·실행 예시(스크립트가 실제로 하는 일)

Cookie·모드는 폼에 따릅니다. 날짜는 **`YYYYMMDD`**(하이픈 없음).

### 예 A: 랭킹(스트리밍·날짜 루프)

| 항목 | 예 |
|------|-----|
| 소스 | 랭킹 |
| 랭킹 유형 | 일간(`daily`) |
| 콘텐츠 | 전체(`all`) |
| 시작일/종료일 | 예: `20240101`～`20240102` |
| 최대 작품 수 | `120` |

각 날 `ranking.php?...&p={p}&format=json`를 `p=1`부터. 각 `illust_id`에 대해 **해당 작품** `pages` → `urls.original`을 끝낸 뒤 1 증가, **120**이면 **전체 작업** 종료. `p=k` 오류(404 포함)면 그날 더 이상 페이지 없음. 예전 방식(상한으로 페이지 수 역산 후 목록만 선행 수집)은 **폐지**.

---

### 예 B: 북마크(페이지마다 즉시 다운로드)

| 항목 | 예 |
|------|-----|
| 소스 | 북마크 |
| 사용자 UID | 예: `12345678` |
| 최대 작품 수 | `50` |

`offset=0`, `limit=48`. `body.works[].id`를 **순서대로** `pages`까지 받고 **50** 작품에서 멈추거나 48 미만이면 다음 없음.

---

### 예 C: 작가(목록 1회, 다운로드만 상한)

| 항목 | 예 |
|------|-----|
| 소스 | 작가 작품 |
| 로그인 UID(`x-user-id`) | 본인 계정 UID |
| 작가 UID | 예: `87654321` |
| 최대 작품 수 | `5` |

`profile/all` 1회, `body.illusts` 키 순으로 **최대 5작품**(작품당 파일 여러 개 가능).

---

### 예 D: 키워드(검색 페이지 단위 스트리밍)

| 항목 | 예 |
|------|-----|
| 소스 | 키워드 |
| 키워드 | 예: `初音ミク` |
| 검색 모드 | 세이프(`safe`) |
| 정렬 | 날짜(`date_d`) |
| 최대 작품 수 | `25` |

`ajax/search/artworks/...`, `p=1,2,…`. `illustManga.data`를 **받으며** 다운로드. **25** 도달 또는 한 페이지 **60 미만**이면 종료.

---

네 가지 `source`를 다룹니다. R18·주간·월간은 URL 의미만 변경.**공통: 상한은 `num_artworks`. 페이징이 있는 모드는 스트리밍(더 필요할 때만 다음 목록 페이지 요청).**

## 주의사항

- 403이 나오면 cookie를 다시 받아보세요. 가끔만 나오면 rate limit일 수 있으니 작업을 몇 번 더 실행하거나, 서프 페이지에서 수동 다운로드하세요.
- Cookie 만료 후에는 다시 받아야 합니다.
- 키워드는 고급 검색 문법 지원 (예: `(Lucy OR 边缘行者) AND 5000users`)
- **인기순**은 Pixiv Premium이 필요합니다. Premium이 아니면 「날짜순」을 사용하세요.
- 최대 작품 수를 적당히 설정해 Pixiv 부하를 줄여주세요.
