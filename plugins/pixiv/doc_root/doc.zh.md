...
- **排行榜**：按榜序（日/周/月/新生/原创/AI 日榜/性别向等）+ 内容类型 + 年龄向组合下载；单日期 `ranking_date`（留空则 Pixiv 返回最新一期）
- **个人收藏**：下载你公开收藏的插画
- **画师作品**：下载指定画师的公开作品
- **关键词搜索**：按关键词搜索并下载
...
- **排行榜**
  - **榜序**（`ranking_mode`）：日榜、周榜、月榜、新生榜、原创榜、AI 日榜、男性向、女性向（不含 R18 后缀；R18 由「年龄向」单独选择）。
  - **内容类型**（`content_mode`）：仅在**日榜 / 周榜 / 月榜 / 新生榜**下显示，可选综合、插画、动图、漫画；**原创榜 / AI 日榜 / 性别向**与站点一致固定为综合，不显示该项。
  - **年龄向**（`age_mode`）：仅在支持 R18 的榜序下显示（日/周/AI 日/男/女）；**月榜 / 新生 / 原创**无 R18，不显示该项。选 R18 时须在「高级设置 → HTTP 头」填写 Cookie，并填写 **用户 UID**（`user_id`，用于 `x-user-id`）。
  - **排行榜日期**（`ranking_date`，`YYYYMMDD`）：留空不传 `date` 参数，由 Pixiv 返回当前最新一期；填写时，日榜为当日统计日，周榜/月榜以该日为基准向前一周或一月（与官网一致）。
  - 维度约束：动图（うごイラ）不能与**月榜、新生榜**组合（脚本会报错）。单榜最多约 **500** 个作品，分页由接口返回的 **`next`** 字段决定下一页页码；`next` 为 `false` 时表示已无下一页。
- **收藏**：用户 UID
- **画师**：用户 UID、画师 UID
...
1. **排行榜**  
   单次统计周期内从 `p=1` 请求 `ranking.php?mode=...&content=...&format=json`（`date` 仅在有 `ranking_date` 时附加）。页内按顺序处理 `illust_id`，**每处理完一个作品计 1**。下一页页码取自响应中的 **`next`**；`next` 为 `false` 或无更多数据时结束。若最终已处理数量仍小于 `num_artworks`，脚本会 **`warn`** 提示数量不足。
...
| 内容类型 | 全部（`all`） |
| 年龄向 | 全年龄（`safe`） |
| 排行榜日期 | 留空（最新）或例如 `20260328` |
| 最大作品数 | `120` |

**流程**  
从 `p=1` 请求  
`https://www.pixiv.net/ranking.php?mode=daily&content=all&p={p}&format=json`（有日期时加 `&date=YYYYMMDD`）。  
对 `contents` 中每个 `illust_id` 下载原图后计 1；满 `120` 或响应中 **`next` 为 `false`** 则停。下一页使用响应里的 **`next`** 作为页码（与官网 JSON 一致）。
...
以上四例对应四种 `source`；排行榜的 `mode` / `content` / `date` / R18 由表单三维度与脚本拼接。**共同点是：以 `num_artworks` 为作品数上限，列表分页模式均为流式。**

## 注意事项

- 排行榜列表请求失败时脚本会结束当前分页循环；若已获取数量不足 `num_artworks`，会输出 **warn** 提示。
