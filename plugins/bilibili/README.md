...
## 二点五、怎么用：UP 主图文 / 我的图文收藏（批量 opus）

**模式** 选 **「UP 主图文」** 或 **「我的图文收藏」** 时，按 B 站图文动态流批量收图：

- **UP 主图文**：在 **UP 主 UID 或主页链接** 里填纯数字 UID（`3985676`）或
  `https://space.bilibili.com/3985676`。对应网页版个人空间的「图文」Tab
  （`space.bilibili.com/<UID>/upload/opus`），按动态流从新到旧翻页。
- **我的图文收藏**：无需填目标——收的是**当前登录账号**自己收藏的图文
  （`space.bilibili.com/<UID>/favlist?fid=opus`），**必须**先在畅游登录 bilibili。
- **最多图文篇数**（默认 20）：本次最多抓几篇；每篇里可能有多张图，各下各的。
- 每篇图文的收图方式与「图文帖（opus）单帖」一致：解析详情页结构化数据，
  拿不到时回退 HTML 抠图。充电专属等有内容门槛的帖子会提示并跳过图片缺失部分。

### 实况图（LivePhoto）

图文里的**实况图**除静态图外还带一段短视频。**「下载实况图视频」**（默认开启）会把
视频轨也一并入库，文件名带 `(live)` 后缀；关闭则只收静态图。该开关对单帖 / UP 主图文 /
收藏 都生效。
...
| 名称 | 作用 | 默认 |
| --- | --- | --- |
| 模式 | 关键词搜索 / 单篇 / UP 主图文 / 我的图文收藏 | 关键词搜索 |
| 专栏 ID 或链接 | **仅单篇模式**：cv 链接 / 数字 id，或 opus 链接 / 长数字 id | （空） |
| UP 主 UID 或主页链接 | **仅 UP 主图文模式**：UID 或 space 链接 | （空） |
| 最多图文篇数 | **UP 主图文 / 收藏模式**：本次最多抓几篇 | 20 |
| 下载实况图视频 | 实况图的视频轨也入库（文件名带 `(live)`） | 开 |
